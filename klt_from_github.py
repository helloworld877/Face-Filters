...
def main():
    ...
    for img_index in range(start_image_index, end_image_index + 1):
        ...
        # full color images:
        (loaded_image_1, pixels_1) = img_helper.load_image(
            "images/sample1-" + str(img_index) + ".png")
        (loaded_image_2, pixels_2) = img_helper.load_image(
            "images/sample1-" + str(img_index + 1) + ".png")
        ...
        for x in range(image_width):
            for y in range(image_height):
                avg = (pixels_1[x, y][0] + pixels_1[x, y]
                       [1] + pixels_1[x, y][2]) / 3.0
        ...
        for x in range(image_width):
            for y in range(image_height):
                avg = (pixels_2[x, y][0] + pixels_2[x, y]
                       [1] + pixels_2[x, y][2]) / 3.0
        ...
        for point_index in range(len(x0_arr)):
            ...
            s_matrix = np.concatenate(
                np.array([transpose_changes_x, transpose_changes_y]), axis=1)
            ...
            img_helper.draw_line(loaded_image_1,
                                 (
                                     line_x0, line_y0,
                                     line_x0 +
                                     velocity_vector_arr[point_index][0][0] *
                                     line_scale_factor,
